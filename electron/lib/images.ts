...
export const createTrayIconWithNotificationDot = (
  dataUrls: string[],
  color?: string
) => {
  // Build a nativeImage with multiple representations, each overlaid
  ...
  // to apply tint before drawing the dot on icon
  if (tint) applyColorToPng(firstPng, tint);
  ...
  dataUrls.forEach((dataUrl, index) => {
    ...
    const buf = dataUrlToBuffer(dataUrl);
    const png = PNG.sync.read(buf);
    if (tint) applyColorToPng(png, tint);
